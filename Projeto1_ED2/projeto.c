  ...
  while (1) {
    ...
    case 1000:
     ...
     // Tempo do vetor aleatorio
      ...
      media = media / 10;
      printf("\nA media de execucao (aleatorio): %f\n\n",media);

      // Chamando função de Marcação de tempo da ordenação Crescente e Decrescente
      // Liberação de memória consta dentro da função chamada
      Marca_tempo(qtd, algoritmo_escolhido);
...
// Função de tempo
void Marca_tempo(int qtd, void (*algoritmo_escolhido)(int*, int)) {
    struct timeval Tempo_antes, Tempo_depois;
    double deltaT;
